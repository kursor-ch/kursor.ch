...
export default function IntroScreen({ onStart }: IntroScreenProps) {
  const diagnosticCount = useMemo(() => {
    const BASE = 1_284;
    const LAUNCH = new Date("2026-04-15").getTime();
    const now = Date.now();
    const days = Math.max(0, Math.floor((now - LAUNCH) / 86_400_000));
    const dailyGrowth = 9 + (days % 4);
    ...
    <>
      ...
      <div className="relative flex flex-col items-center min-h-[calc(100vh-60px)] justify-center text-center px-6 py-4 md:py-10 overflow-hidden">
          ...
          <div className="border border-gray-100 rounded-xl bg-white/40 px-5 py-4">
            ...
            <div className="flex flex-col gap-2.5">
              <div className="flex items-center gap-2.5 text-xs text-gray-500">
                ...
                Score de difficulté personnalisé
              </div>
              <div className="flex items-center gap-2.5 text-xs text-gray-500">
                <svg
                  className="w-4 h-4 text-amber flex-shrink-0"
                ...
                Estimation de durée de recherche
              </div>
              <div className="flex items-center gap-2.5 text-xs text-gray-500">
                <svg
                  className="w-4 h-4 text-amber flex-shrink-0"
        ...
        <blockquote className="relative mt-6 max-w-xs mx-auto text-center animate-intro-testimonial">
          <p className="text-sm text-gray-400 italic leading-relaxed">
            &laquo;&nbsp;En quelques minutes j&apos;ai compris pourquoi mon
            dossier ne passait pas. Le fix m&apos;a pris 48h.&nbsp;&raquo;
          </p>
          <footer className="mt-2 text-xs text-gray-300">
            — Camille R., infirmière, Lausanne
          </footer>
        </blockquote>
      </div>
      ...
      <section className="relative px-6 py-10 md:py-14 text-center border-t border-gray-100">
        ...
        <p className="text-gray-500 text-sm md:text-base max-w-md mx-auto mb-8 leading-relaxed">
          Réservez un appel gratuit de 15 minutes avec un expert du logement en
          Suisse romande.
        </p>
        <a
        ...
        <p className="mt-4 text-xs text-gray-400">
          Sans engagement. Vos données ne sont partagées qu&apos;avec votre
          accord explicite.
        </p>
      </section>
